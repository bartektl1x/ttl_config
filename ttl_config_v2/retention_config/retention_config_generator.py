...
class RetentionConfigGenerator:
    ...
    def generate_retention_config(
        ...
        *,
        ...
        write_retention_config is called.
        """
    ...
    def write_retention_config(
        self,
        rules: RetentionRules,
    ) -> None:
        """Replace the destination with the complete generated rules snapshot.

        Pass the rules returned by generate_retention_config. Omitted targets
        ...
        """
        table_name = self._resolve_retention_config_table()
        retention_config = self._prepare_retention_config(rules)
        destination_exists = self._spark.catalog.tableExists(
            table_name.sql_identifier
        )

        if destination_exists:
            self._validate_retention_config_table(
                table_name,
    ...
    def _inherit_retention_rules(
        ...
        from retention_config.retention_rule_inheritance import (
    ...
    def _resolve_retention_config_table(self) -> TableName:
        return TableName(
    ...
    def _validate_retention_config_table(
        self,
        table_name: TableName,
        expected_schema: StructType,
    ) -> None:
        self._validate_retention_config_table_type(table_name)
        self._validate_retention_config_table_format(table_name)
        self._validate_retention_config_table_schema(
            table_name,
            expected_schema,
        )

    def _validate_retention_config_table_type(
        self,
        table_name: TableName,
    ) -> None:
    ...
    def _validate_retention_config_table_format(
        self,
        table_name: TableName,
    ) -> None:
        identifier = table_name.sql_identifier
    ...
    def _validate_retention_config_table_schema(
        self,
        table_name: TableName,
        expected_schema: StructType,
    ) -> None:
